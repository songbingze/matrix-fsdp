...
MatrixFSDP provides an FSDP2-like `fully_shard(...)` API with MatrixShard-aware
layouts. The runtime manages parameter unshard/reshard, backward gradient
reduction, and optimizer finalization behind the normal PyTorch training loop:
...
Create optimizers after sharding so they see the sharded parameter views.
