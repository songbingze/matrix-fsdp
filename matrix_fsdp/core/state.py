...
class MatrixShardedState:
    ...
    MatrixFSDP keeps runtime storage as ordinary local tensors. ``MatrixShard``
    metadata records how that local tensor maps into the logical global tensor;
    PyTorch DTensor wrappers are intentionally not cached here since they may
    allocate or retain full-size local storage for custom placements.
    """
