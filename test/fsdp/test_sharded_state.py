...
class MatrixShardedStateTest(unittest.TestCase):
    def test_mesh_state_keeps_local_tensor_and_matrix_metadata(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.assertIsNone(metadata["dtensor_spec"])
            finally:
                dist.destroy_process_group()

    def test_2d_mesh_state_records_replicate_and_shard_mesh_metadata(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            dist.init_process_group("gloo", init_method=f"file://{tmpdir}/init", rank=0, world_size=1)
            try:
                mesh = DeviceMesh(
                ...
                metadata = state.as_metadata()
                self.assertIsNone(metadata["dtensor_spec"])
                self.assertEqual(metadata["device_mesh"]["mesh_dim_names"], ("dp_replicate", "dp_shard"))
                self.assertEqual(
                    metadata["matrix_shard"],
